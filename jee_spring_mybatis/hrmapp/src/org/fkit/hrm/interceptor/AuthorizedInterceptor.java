...
/** 判断用户权限的Spring MVC的拦截器:重写拦截器的三个方法 */
public class AuthorizedInterceptor implements HandlerInterceptor {
     ...
     *  如果preHandle返回值为true, 则会继续执行postHandle和afterCompletion */
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (!flag){                                                                             /** 拦截请求 */
			User user = (User) request.getSession().getAttribute(HrmConstants.USER_SESSION);	/** 1.获取session中的用户  */
	...
	/** 这个方法在preHandle方法返回true时才会执行, 执行时间是在处理器进行处理之后, 也就是在Controller的方法调用之后执行 */
	@Override
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView mv) throws Exception {}

	/** 这个方法在preHandle方法返回true时才会执行, 它将在整个请求完成之后执行, 主要用于清理资源 */
	@Override
