...
public class PagerTag extends SimpleTagSupport {
	...
	@Override															/**  在页面上引用自定义标签就会触发一个标签处理类 */
	public void doTag() throws JspException, IOException {
		
		StringBuilder res = new StringBuilder();						/** 定义它拼接是终的结果 */
		StringBuilder str = new StringBuilder();						/** 定义它拼接中间的页码 */
		
		if (recordCount > 0){   										// 1499/15  = 100	/** 判断总记录条数 */
			totalPage = (this.recordCount - 1) / this.pageSize + 1; 	/** 需要显示分页标签，计算出总页数 需要分多少页 */
			if (this.pageIndex == 1){ 									// 首页	/** 判断上一页或下一页需不需要加a标签 */
				str.append("<span class='disabled'>上一页</span>");
				this.calcPage(str);										/** 计算中间的页码 */
				if (this.pageIndex == totalPage){						/** 下一页需不需要a标签 */
					str.append("<span class='disabled'>下一页</span>");	/** 只有一页 */
				}else{
			...
			int startNum = (this.pageIndex - 1) * this.pageSize + 1;												/** 开始条数 */
			int endNum = (this.pageIndex == this.totalPage) ? this.recordCount : this.pageIndex * this.pageSize;	/** 结束条数 */
	...
	private void calcPage(StringBuilder str) {
		if (this.totalPage <= 11){													/** 判断总页数 */
			for (int i = 1; i <= this.totalPage; i++){								/** 一次性显示全部的页码 */
				if (this.pageIndex == i){
					str.append("<span class='current'>"+ i +"</span>");				/** 当前页码 */
				}else{
