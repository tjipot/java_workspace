...
//		t.testSelectEmployeeLikeName(session);
	...
	public void testSelectEmployeeByIdLike(SqlSession session){
		EmployeeMapper em = session.getMapper(EmployeeMapper.class);	// 获得EmployeeMapper接口的代理对象;
		HashMap<String, Object> params = new HashMap<String, Object>();	// 创建一个HashMap存储参数;
		params.put("id", 1);											// 设置id属性;
		// 调用EmployeeMapper接口的selectEmployeeByIdLike方法: 对应select或update标签中的id值;
		List<Employee> list = em.selectEmployeeByIdLike(params);
		list.forEach(employee -> System.out.println(employee));			// 查看返回结果;
	}
...
//		params.put("password", "123456");
	...
	public void testUpdateEmployeeIfNecessary(SqlSession session){
		...
		employee.setLoginname("mary");	// 设置需要修改的属性;
		employee.setPassword("123");	// 设置需要修改的属性;
		employee.setName("玛丽");		// 设置需要修改的属性;
